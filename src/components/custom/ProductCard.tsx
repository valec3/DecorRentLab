      ...
      <div className="flex flex-col h-full">
        <Link href={`/producto/${producto.slug}`}>
          <div className="relative aspect-4/5 overflow-hidden bg-crema-oscuro/30">
            ...
            <div className="absolute top-2 left-2 md:top-4 md:left-4 flex flex-col gap-1 z-10">
              ...
              {!producto.disponible && (
                <Badge variant="unavailable" className="text-[7px] md:text-[9px] px-1.5 md:px-2.5">Agotado</Badge>
              )}
            </div>
          </div>
        </Link>
        ...
        <div className="p-3 md:p-6 flex flex-col flex-1">
          <div className="flex justify-between items-start mb-1 md:mb-2">
            <span className="text-dorado-oscuro/60 uppercase tracking-[0.2em] font-bold text-[7px] md:text-[8px]">
          ...
          <Link href={`/producto/${producto.slug}`}>
            <h3 className="font-serif text-sm md:text-lg text-carbon mb-1 group-hover:text-dorado transition-colors duration-300 line-clamp-1 leading-tight">
              {producto.nombre}
            </h3>
          </Link>
          
          <p className="text-gris-calido/70 text-[9px] md:text-[11px] font-light mb-auto line-clamp-1 leading-relaxed">
            {producto.descripcionCorta}
          </p>
  
          <div className="flex gap-1.5 md:gap-2 mt-4 md:mt-6">
            <Link href={`/producto/${producto.slug}`} className="flex-1">
              <Button variant="outline" size="sm" className="w-full text-[8px] md:text-[10px] h-8 md:h-9 border-borde/40 text-carbon/60 hover:text-dorado">
                Detalles
              </Button>
            </Link>
            <a href={whatsappUrl} target="_blank" rel="noopener noreferrer" className="flex-1">
              <Button variant="primary" size="sm" className="w-full text-[8px] md:text-[10px] h-8 md:h-9">
                Cotizar
              </Button>
            </a>
          </div>
