...
export default function Home() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [resDestacados, resCats, resTestimonios, resFaqs] =
          await Promise.all([
    ...
    <div className="overflow-x-hidden">
      ...
      >
        ...
        >
          <Image
            ...
            priority
          />
          <div className="absolute inset-0 bg-linear-to-b from-black/50 via-black/20 to-crema/20" />
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20 relative z-10 text-center">
          ...
          <ScrollReveal delay={0.15} width="100%">
            <h1 className="font-serif text-5xl md:text-7xl lg:text-8xl text-white mb-8 leading-[1.1] drop-shadow-2xl">
              ...
              <span className="italic font-display text-white/95 text-4xl md:text-6xl lg:text-7xl block mb-2">
          ...
          <ScrollReveal delay={0.45} width="100%">
            <div className="flex flex-col sm:flex-row items-center justify-center gap-6 w-full max-w-lg mx-auto">
              <Link href="/catalogo">
                <Button
                  size="lg"
                  className="w-full sm:w-auto px-8 md:px-10 py-5 text-base md:text-lg whitespace-nowrap"
                >
          ...
          <div className="flex flex-col md:flex-row md:items-end justify-between mb-12 gap-6">
            <ScrollReveal direction="right" width="100%">
              <div className="text-center md:text-left">
                <span className="text-dorado text-[10px] tracking-premium font-bold mb-5 block">
                  Colecciones
                </span>
                <h2 className="font-serif text-4xl md:text-6xl text-carbon leading-tight">
                  Explora por{" "}
                  <span className="italic font-display text-carbon/80">
                    Categoría
                  </span>
                </h2>
              </div>
            ...
            <ScrollReveal direction="down" width="100%">
              <h2 className="font-serif text-4xl md:text-5xl text-carbon mb-6">
                {homeContent.howItWorks.title.split(", ").map((text, i) => (
                  <span
                    key={i}
                    className={i === 1 ? "italic font-display" : ""}
                  >
                    {text}
                    {i === 0 ? ", " : ""}
                  </span>
              ...
              <ScrollReveal key={i} delay={i * 0.2}>
                <div className="group p-10 bg-white border border-borde rounded-[2.5rem] hover:shadow-strong transition-all duration-500 text-center relative overflow-hidden">
                  ...
                  <div className="mb-6 inline-flex p-5 rounded-3xl bg-crema group-hover:scale-110 transition-transform duration-500">
                    {i === 0 ? (
                      <Sparkles className="w-8 h-8 text-dorado" />
                    ) : i === 1 ? (
                      <MessageSquare className="w-8 h-8 text-dorado" />
                    ) : (
                      <Truck className="w-8 h-8 text-dorado" />
                    )}
                  </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
            ...
            >
              <div className="relative group h-full">
                ...
                >
                  <span className="block text-4xl font-serif text-dorado mb-1">
                    {homeContent.philosophy.stats.value}
                  </span>
                  <span className="text-xs uppercase tracking-widest text-carbon/60">
                    {homeContent.philosophy.stats.label}
                  </span>
                </motion.div>
              </div>
            </ScrollReveal>
                ...
                {homeContent.philosophy.items.map((item, i) => (
                  <ScrollReveal key={i} delay={i * 0.15}>
                    <div className="space-y-2">
                      ...
                      <p className="text-sm text-gris-calido leading-relaxed">
                        {item.description}
                      </p>
                    </div>
                  </ScrollReveal>
      ...
      {testimonios.length > 0 && (
        <section className="py-20 bg-crema relative overflow-hidden">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="text-center mb-12">
              <ScrollReveal direction="down" width="100%">
                <span className="text-dorado text-xs uppercase tracking-[0.3em] font-bold block mb-4">
                  Experiencias
                </span>
                <h2 className="font-serif text-4xl md:text-5xl text-carbon mb-2">
                  La voz de la{" "}
                  <span className="italic font-display">elegancia</span>
                </h2>
              </ScrollReveal>
            </div>
            <ScrollReveal delay={0.3} width="100%">
      ...
      <section className="py-24 bg-white">
        <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
          <ScrollReveal direction="down" width="100%">
            <div className="text-center mb-12">
              <h2 className="font-serif text-4xl text-carbon mb-6">
                Preguntas <span className="italic font-display">Clave</span>
              </h2>
              <p className="text-gris-calido">
                Todo lo que necesitás saber antes de empezar el viaje con
                nosotros.
              </p>
            </div>
          </ScrollReveal>
                ...
                <h2 className="font-serif text-4xl md:text-6xl text-white mb-10 leading-[1.2]">
                  {homeContent.cta.title.split(" tu ").map((text, i) => (
                    <span key={i}>
                      ...
                      <span
                        className={
                          i === 1 ? "text-dorado italic font-display" : ""
                        }
                      >
                        {text}
                      </span>
                ...
                <div className="flex flex-col sm:flex-row items-center justify-center gap-6">
                  ...
                  <Link href="/contacto" className="w-full sm:w-auto">
                    <Button
                      variant="outline"
                      size="lg"
                      className="w-full sm:w-auto px-12 border-white/20 text-white hover:bg-white/10"
                    >
